...
find_sec = [[sg.Text('File path\nfor results:\n'), sg.InputText(enable_events=True, key="-FOLDER2-"), sg.FolderBrowse()],
    [sg.Text('Pattern:'), sg.InputText(key="-PATTERN-")],
...
# Create the window
window = sg.Window("Find all Substrings (PART 1)", layout)
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        x = values["-FOLDER-"]
        if x == '':
            # tree = SuffixTree(make_string_ready(values["-FOLDER1-"][:-1]))
        ...
        a = tree.find_all_sub(values["-PATTERN-"])
        if values["-FOLDER2-"]:
